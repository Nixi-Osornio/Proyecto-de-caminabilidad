...
if ($calle !== '' && $clasificacion !== '') {
    ...
    $query = "SELECT COUNT(*) AS total
              FROM fm_captura
              WHERE nomvial = $1 AND practicom_clas = $2";
    ...
    $query = "SELECT practicom_clas, COUNT(*) AS total
              FROM fm_captura
              WHERE nomvial = $1
              GROUP BY practicom_clas
              ORDER BY practicom_clas";
    $result = pg_query_params($conexion, $query, [$calle]);

    $labels = [];
    $values = [];
    while ($row = pg_fetch_assoc($result)) {
        $labels[] = $row['practicom_clas'];
        $values[] = (int)$row['total'];
    }
