...
if ($calle !== '' && $clasificacion !== '') {
    ...
    $query = "SELECT COUNT(*) AS total
              FROM fm_captura
              WHERE nomvial = $1 AND accesible_clas = $2";
    ...
    $query = "SELECT accesible_clas, COUNT(*) AS total
              FROM fm_captura
              WHERE nomvial = $1
              GROUP BY accesible_clas
              ORDER BY accesible_clas";
    $result = pg_query_params($conexion, $query, [$calle]);

    $labels = [];
    $values = [];
    while ($row = pg_fetch_assoc($result)) {
        $labels[] = $row['accesible_clas'];
        $values[] = (int)$row['total'];
    }
