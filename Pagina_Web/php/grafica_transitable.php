...
if ($calle !== '' && $clasificacion !== '') {
    ...
    $query = "SELECT COUNT(*) AS total
              FROM fm_captura
              WHERE nomvial = $1 AND transitable_clas = $2";
    ...
    $query = "SELECT transitable_clas, COUNT(*) AS total
              FROM fm_captura
              WHERE nomvial = $1
              GROUP BY transitable_clas
              ORDER BY transitable_clas";
    $result = pg_query_params($conexion, $query, [$calle]);

    $labels = [];
    $values = [];
    while ($row = pg_fetch_assoc($result)) {
        $labels[] = $row['transitable_clas'];
        $values[] = (int)$row['total'];
    }
